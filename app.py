...
st.markdown("""
    ...
    /* Centralización del Spinner y mensajes de carga */
    ...
    /* Botón azul alineado a la derecha */
    div.stButton > button {
...
@st.cache_data
def load_data():
    # Carga de calificaciones y catálogo de películas (MovieLens 100k)
    ...
    # Creación de la lista de géneros para visualización
...
# Función SoftImpute con corrección para ocultar mensaje automático de caché
...
if 'random_sample' not in st.session_state:
    # Selección aleatoria de 20 películas para calificar
...
if st.session_state.view == 'home':
    ...
    # Proceso de calificación (Carrusel)
    else:
...
# --- VISTA: RESULTADOS ---
    ...
    with st.spinner(t['loading']):
        ...
        # Centrado de datos por la media de cada usuario
        ...
        # Cruce con catálogo de películas y filtrado de las ya vistas
    ...
    # Visualización de las 10 mejores recomendaciones
    ...
    # Reinicio de la sesión manteniendo caché pesado
    if st.button(t['restart'], use_container_width=True):
        st.session_state.view = 'home'
        st.session_state.my_ratings = {}
        st.session_state.current_idx = 0
        st.session_state.started = False
        if 'completed_matrix_soft' in st.session_state: del st.session_state.completed_matrix_soft
        st.session_state.random_sample = df_movies.sample(20).reset_index(drop=True)
        st.rerun()